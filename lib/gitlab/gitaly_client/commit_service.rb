module Gitlab
  module GitalyClient
    class CommitService
      ...
      def filter_shas_with_signature(shas)
        request = Gitaly::FilterShasWithSignatureRequest.new(repository: @gitaly_repo)

        enum = Enumerator.new do |y|
          shas.each_slice(20).to_a do |revs|
            request.shas = GitalyClient.encode_repeated(revs)

            y.yield request

            request = Gitaly::FilterShasWithSignatureRequest.new()
          end
        end

        response = GitalyClient.call(@repository.storage, :commit_service, :filter_shas_with_signature, enum)

        response.flat_map.shas
      end
